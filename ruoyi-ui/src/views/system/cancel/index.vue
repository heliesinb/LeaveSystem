...
<!--      </el-col>-->
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:cancel:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:cancel:export']"
        >导出</el-button>
      </el-col>
...
<!--        </el-form-item>-->
        <el-form-item label="请假类型" prop="leaveType" hidden>
        ...
        <el-form-item label="请假时长" prop="leaveTimeType" hidden>
        ...
        <el-form-item label="开始时间" prop="startTime" hidden>
          <el-date-picker clearable
        ...
        <el-form-item label="结束时间" prop="endTime" hidden>
          <el-date-picker clearable
        ...
        <el-form-item label="是否离校" prop="isLeaveSchool" hidden>
...
<!--        </el-form-item>-->
        ...
        <el-form-item label="审批状态" prop="approvalType" v-if="flag==0" hidden>
          <el-select v-model="form.approvalType" placeholder="请审批">
            <el-option
              v-for="dict in dict.type.sys_approval_type"
              :key="dict.value"
              :label="dict.label"
        ...
        <el-form-item label="销假" prop="leaveStatus" v-if="flag==0">
          <el-select v-model="form.leaveStatus" placeholder="请销假">
            <el-option
              v-for="dict in dict.type.sys_leave_status"
              :key="dict.value"
              :label="dict.label"
...
<!--        </el-form-item>-->
        <el-form-item label="备注" prop="remark" hidden>
          <el-input v-model="form.remark" type="textarea" placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="审批意见" prop="approvalAdvice" hidden >
          <el-input v-model="form.approvalAdvice" type="textarea" placeholder="请输入意见" />
        </el-form-item>
      </el-form>
...
<script>
import { listLeave, getLeave, delLeave, addLeave, updateLeave, listApproval } from "@/api/system/cancel";
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getLeave(leaveId).then(response => {
        ...
        this.cancel = this.form.leaveStatus;
        this.open = true;
        this.title = "销假操作";
        this.flag = 0;
      });
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.leaveId != null) {
            if(this.cancel == 2){
              this.$modal.msgError("已销假，请勿重复操作");
            }else{
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除请假信息编号为"' + leaveIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cancel/export', {
        ...this.queryParams
